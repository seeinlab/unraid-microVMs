#!/bin/bash
# Fix nginx-fc init - no serial shell for Firecracker
/etc/rc.d/rc.microvm stop_vm nginx-fc
sleep 3
mkdir -p /tmp/fixfc
mount /mnt/mtier/microvms/nginx-fc/rootfs.raw /tmp/fixfc

cat > /tmp/fixfc/init << 'INITEOF'
#!/bin/sh
